...
public abstract class ReferralDoctorValidatorBase<T> : AbstractValidator<T> where T : IReferralDoctorFields
{
    protected ReferralDoctorValidatorBase()
    {
        ...
        RuleFor(x => x.Email).EmailAddress().WithErrorCode(ErrorCodes.Shared.InvalidEmail).MaximumLength(200).WithErrorCode(ErrorCodes.Shared.TextTooLong).When(x => !string.IsNullOrWhiteSpace(x.Email));
