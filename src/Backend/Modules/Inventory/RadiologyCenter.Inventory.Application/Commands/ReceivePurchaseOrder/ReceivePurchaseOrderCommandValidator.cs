...
public class ReceivePurchaseOrderCommandValidator : AbstractValidator<ReceivePurchaseOrderCommand>
{
    ...
    {
        RuleFor(x => x.PurchaseOrderId).NotEmpty().WithErrorCode(ErrorCodes.PurchaseOrderIdRequired);
        ...
        {
            line.RuleFor(i => i.ItemId).NotEmpty().WithErrorCode(ErrorCodes.ReceiveLineItemIdRequired);
            line.RuleFor(i => i.Quantity).GreaterThan(0).WithErrorCode(ErrorCodes.ReceiveLineQuantityMustBePositive);
            line.RuleFor(i => i.LotNumber).NotEmpty().WithErrorCode(ErrorCodes.ReceiveLineLotNumberRequired).MaximumLength(100).WithErrorCode(ErrorCodes.ReceiveLineLotNumberTooLong);
